...
from UI.UI_HistoryInfoWidget import Ui_HistroyInfoView
...
class HistoryIndexWidget(QWidget):
    ...
    def __init__(self,mainWin=MainWindow):
       ...
       #隐藏第一列
        self.ui.History_treeWidget.hideColumn(1)
        ...
        self.query = "SELECT Title,Author,Summary,Pree,PublishYear,ID from History  "
    ...
    def switchArea_callback(self):
        ...
        if item == '历史文献库':
            ...
            code = int(self.ui.History_treeWidget.currentItem().text(1))
            print(code)
            if code in range(1,3):
                self.condition = " WHERE Class1 LIKE  \'%%%s%%\' " % (item)
            elif code in range(10,12):
                self.condition = " WHERE Class1 LIKE \'方志\' AND Class2 LIKE \'%%%s%%\' " % (item)
            elif code  in range(12,14):
                self.condition = " WHERE Class1 LIKE \'古代著述\' AND Class2  LIKE \'%%%s%%\' " % (item)
            elif code in range(20,23):
                self.condition = " WHERE Class1 LIKE \'方志\' AND Class2 LIKE \'古代方志\' AND Class3 LIKE \'%%%s%%\' " % (item)
            elif code in range(23,27):
                self.condition = " WHERE Class1 LIKE \'方志\' AND Class2 LIKE \'近代方志\' AND Class3 LIKE \'%%%s%%\' " % (item)
            else:
                self.condition = ""
    ...
    def openByDoubleClick_callback(self,index):
        ...
        historyInfoWidget.setSeriesName(self.sqlQuery.value("SeriesName"))
        historyInfoWidget.setSummary(self.sqlQuery.value("Summary"))
        historyInfoWidget.setKeyword(self.sqlQuery.value("Keyword"))
        historyInfoWidget.setPress(self.sqlQuery.value("Pree"))
        historyInfoWidget.setCategory(self.sqlQuery.value("Category"))
        historyInfoWidget.setVolume(self.sqlQuery.value("Volume"))
        historyInfoWidget.setCe(self.sqlQuery.value("Book"))
        historyInfoWidget.setPages(self.sqlQuery.value("Pages"))
        historyInfoWidget.setPubDate(self.sqlQuery.value("PublishYear"))

        historyInfoWidget.setFL(self.sqlQuery.value("Class1")+ ' | ' + self.sqlQuery.value("Class2") + ' | ' + self.sqlQuery.value("Class3"))

        self.mainWin.cenTab.addTab(historyInfoWidget,title[0:11])
...
class HistoryInfoWidget(QWidget):
    ...
    def getPDFStream(self,md5):
        sqQuery = "select FileBinary from  HistoryFile where md5=?"
